...
void loop() {
  ...
  if ((distance == 0.0) || (distance > _DIST_MAX)) {
      ...
      digitalWrite(PIN_LED, 1);       // LED OFF
  } else if (distance >= 100 && distance <= 200) {  // In desired Range
      brightness = 2.5 * (200-distance);
      analogWrite(PIN_LED, brightness);       // LED ON      
  } else if (distance > 200 && distance <= 300) {  // In desired Range
      brightness = 2.5 * (distance-200);
      analogWrite(PIN_LED, brightness);       // LED ON      
  }
